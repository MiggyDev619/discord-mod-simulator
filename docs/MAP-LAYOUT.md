...
**Walls + tower + desk + monitor (Phase 5 base):**

| Element | Position relative to lane | Size | Color | Material |
|---|---|---|---|---|
| `MapPolish_WallLeft` / `WallRight` | flank lane (perpendicular offset 18 studs) | (1, 8, laneLen+30) | dark slate | Slate |
| `MapPolish_WallLeftStrip` / `WallRightStrip` | yellow Neon coping ABOVE each wall | (1.4, 0.5, laneLen+30) | brand yellow | Neon |
| `MapPolish_ServerTower` | 8 studs behind `ServerZone` | (8, 16, 8) | dark grey | DiamondPlate |
| `MapPolish_TowerLED1..3 / Back` | 6 horizontal yellow LED strips around the tower (3 front, 3 back) | (8.1, 0.4, 0.3) each | yellow | Neon |
| `MapPolish_ModeratorDesk` + `MonitorBack` + `MonitorScreen` | behind `EnemyStart` — desk with Discord-blurple monitor | desk: 6×3×3 | wood / dark plastic / blurple Neon | Wood / SmoothPlastic / Neon |

**Decor (added later for "more alive" feel):**

| Element | Purpose |
|---|---|
| `MapPolish_LaneStripe` | thin blurple Neon strip running lane center, dim (0.4 transparency). Reads as a runway showing the data path. |
| `MapPolish_Pillar1L/R/2L/R` + `PillarRing1L/R/2L/R` | 4 dark cylindrical pillars at the lane sides (1/3 + 2/3 along, 8 studs lateral offset). Topped with blurple Neon rings. CanCollide so enemies physics-bounce around them. |
| `MapPolish_WumpusBody/Head/Eye/Pupil/Beanie` | 5-part Discord Wumpus mascot (blurple ball body + head + white eye + black pupil + red beanie) next to the moderator's desk. |
| `MapPolish_SideDesk/SideChairSeat/SideChairBack/SideMonitorBack/SideMonitorScreen` | empty "abandoned coworker" workstation on the opposite side of the lane from Wumpus. Red Neon screen reads as "down station." |
| `MapPolish_PacketEmitter` | invisible anchor part at the tower base with a ParticleEmitter — slow blurple data-packet particles drift up around the tower. Self-running, no script needed. |
| `MapPolish_TowerAnimator` | runtime Script (auto-generated by Apply) that pulses the TowerLED parts via TweenService — 1.6s sine in/out brightness loop. Runs in Play mode. |

Total: ~30 polish parts + 1 ParticleEmitter + 1 animator Script. All anchored or self-contained, all parented to `Workspace.Map`.
