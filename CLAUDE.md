...
| Object | Source | Notes |
|---|---|---|
| `ReplicatedStorage.Shared.Remotes` + all `RemoteEvent`s | `src/ReplicatedStorage/Shared/Remotes.model.json` | Single file owns the whole subtree. Current events: `HealthChanged`, `GameOver`, `GameWon`, `BanEnemy`, `MuteEnemy`, `TimeoutEnemy`, `PurchaseUpgrade`, `WaveStarted`, `WaveBreak` |
...
- **Temporary enemy state lives on attributes.** Mute sets `MutedUntil` (number) + `OriginalColor` (Color3); Timeout sets `FrozenUntil` + `OriginalColor`. `EnemySpawner`'s heartbeat is the sole reader — it applies the active effect's speed modifier each frame and restores color on expiry. When statuses stack, the stronger wins (Timeout overrides Mute, and when it expires EnemySpawner drops the enemy back to mute color if still muted, else to `OriginalColor`). Handlers write the attribute; EnemySpawner reads it and cleans up — one writer, one reader. Future status effects (Stun, DoT) should follow the same pattern.
