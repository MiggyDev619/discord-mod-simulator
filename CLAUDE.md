...
| Object | Source | Notes |
|---|---|---|
| `ReplicatedStorage.Shared.Remotes` + all `RemoteEvent`s | `src/ReplicatedStorage/Shared/Remotes.model.json` | Single file owns the whole subtree. Current events: `HealthChanged`, `GameOver`, `GameWon`, `BanEnemy`, `MuteEnemy`, `TimeoutEnemy`, `KickEnemies`, `PurchaseUpgrade`, `WaveStarted`, `WaveBreak`, `EnemyCountChanged` |
| `StarterGui.MainUI` as `ScreenGui` | `src/StarterGui/MainUI/init.meta.json` | `ResetOnSpawn = false` |
| `MainUI` children (`HealthLabel`, `CurrencyLabel`, `WaveLabel`, `UpgradeButton`, `CooldownPanel`) | `src/StarterGui/MainUI/*.model.json` | **Fully Rojo-owned** including Size/Position/AnchorPoint/colors/Font/TextSize plus nested `UIPadding`/`UICorner` children. Don't style in Studio — edit the `.model.json`. `HealthLabel` is a `Frame` (not a TextLabel) with named `Fill` + `Label` children driving the bar. `CooldownPanel` is a `Frame` with named `Ban`/`Mute`/`Timeout`/`Kick` slot children, each with a `Letter` and `Timer` TextLabel. |
...
- **Tool LocalScripts publish cooldown state via client-set Player attributes.** Each tool sets `<Tool>ReadyAt = tick() + cooldown` on `Players.LocalPlayer` immediately after a successful activation (`BanReadyAt`, `MuteReadyAt`, `TimeoutReadyAt`, `KickReadyAt`). These are client-only — they don't replicate to the server (server already has its own cooldown gating where needed). `ClientMain` reads them per-frame to drive the `CooldownPanel`. New tools should follow the same `<Tool>ReadyAt` naming.
...
- **Server-authoritative for destructive actions.** Ban Hammer LocalScript picks a target and fires `BanEnemy:FireServer(enemyPart)`. Server validates (part has `IsEnemy == true`, is a descendant of workspace, player is within `BAN_RANGE * 1.5`, server-side cooldown from `player:GetAttribute("BanCooldown")`) before destroying and rewarding. Keep this pattern for new tools — never let the client delete or damage enemies directly. **Exception: Kick aim.** The Kick cone direction is sent by the client (`Workspace.CurrentCamera.CFrame.LookVector`, flattened) because the server can't read camera state and using `HumanoidRootPart.CFrame.LookVector` aimed at stale body-rotation directions, breaking the UX. Server still validates the vector is roughly unit-length and horizontal. The kick is non-destructive (no damage, no rewards) so the worst-case spoof is "kicked enemies behind you" — acceptable in this casual game.
- **Enemies are identified by the `IsEnemy` attribute, not by Name.** Each enemy also carries a `Reward` attribute so coin award is data-driven. If you add a new enemy type, set both attributes when spawning — every matcher in the codebase keys off them. Per-enemy behavior state (e.g. Teleporter's `nextTeleport` timer, Splitter's typeName check on ban) lives on the spawner's `activeEnemies` data table next to `damageCooldown`, not on attributes — attributes are reserved for status effects that need to be visible to handler scripts. Type-specific ban-path behavior (e.g. Splitter spawning children before destroy) lives in the `EnemySpawner` ban handler before the `HitFlash`/`BanEffect` calls — keyed off `enemyPart.Name == "<TypeName>"`. `spawnEnemy` accepts optional `spawnPos`/`sizeOverride`/`speedOverride` for derived spawns (children of a Splitter); RoundManager only ever passes type+multiplier. Children should be a distinct type that RoundManager never rolls so they can't recurse (`SplitterChild` pattern).
- **Temporary enemy state lives on attributes.** Mute sets `MutedUntil` + `OriginalColor`; Timeout sets `FrozenUntil` + `OriginalColor`; Kick sets `KickedUntil` + `KickVelocity` (Vector3). `EnemySpawner`'s heartbeat is the sole reader — it applies the active effect's speed or velocity each frame and cleans up attributes on expiry. Priority: **Kick (impulse) > Timeout (freeze) > Mute (slow) > normal seek**. While kicked, every other per-frame system (mute/freeze/seek/zone damage) is skipped until the window closes. When Timeout expires with Mute still active, color drops back to mute blue, not OriginalColor. Handlers write the attribute; EnemySpawner reads and clears — one writer, one reader. Future status effects (Stun, DoT) should follow the same pattern.
